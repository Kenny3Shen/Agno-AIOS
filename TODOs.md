...
- 批量 upsert 的「写入」数按已验证指标计数，不依赖 psycopg 对该语句返回的不可用 `rowcount`。
...
- 通知流对 `background_task` / status=error 即时 toast，并使用错误态图标。
...
- CVE 页：进入即加载最近条目；搜索防抖；更新成功 toast 展示新增/删除数并自动刷新列表。

## 已完成：CVE 更新中重复请求 409

- `POST /api/cve/update` 在文件锁冲突时返回 409（不再伪装 500），与 Collect crawl 锁语义一致。
...
- 重试横幅只提示 Esc/发送区停止，不再并排第二个「停止生成」按钮。
