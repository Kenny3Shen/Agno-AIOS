  ...
  return <main className="page"><PageHeader title="CVE Intelligence" description="搜索本地漏洞情报并维护数据索引" />
    <Card className="workbench-card"><Space className="cve-toolbar" wrap>
      <Space.Compact className="cve-search-control">
        <Input value={query} onChange={(event) => setQuery(event.target.value)} onPressEnter={() => search.mutate()} placeholder="CVE ID、产品或关键词" />
        <Button type="primary" icon={<SearchOutlined />} loading={search.isPending} onClick={() => search.mutate()}>搜索</Button>
      </Space.Compact>
