...
export function MemoryPage() {
  ...
  const metadata = selected ? [
    { key: 'memory', label: 'Memory', children: <Typography.Paragraph className="memory-metadata-text">{selected.memory || '-'}</Typography.Paragraph> },
    ...
    { key: 'topics', label: 'Topics', children: <Space wrap>{selected.topics?.length ? selected.topics.map((topic) => <Tag key={topic}>{topic}</Tag>) : '-'}</Space> },
    { key: 'input', label: 'Input', children: <Typography.Paragraph className="memory-metadata-text">{memoryInput.text || '-'}</Typography.Paragraph> },
    ...(Object.keys(memoryInput.context).length > 0 ? [{ key: 'context', label: 'Context', children: <JsonValueCard value={memoryInput.context} title="Additional context" /> }] : []),
  ...
  return <main className="page">
    ...
    <Modal open={Boolean(editing)} footer={null} onCancel={() => setEditing(null)} title="编辑记忆" destroyOnHidden>
      {editing && <Form layout="vertical" initialValues={{ memory: editing.memory, topics: editing.topics?.join(', ') }} onFinish={async ({ memory, topics }: { memory: string; topics: string }) => { await updateMemory(editing.id, memory, topics.split(',').map((value) => value.trim()).filter(Boolean)); message.success('记忆已更新'); setEditing(null); setSelected(null); await refresh() }}>
        <Form.Item name="memory" label="Memory" rules={[{ required: true }]}><Input.TextArea rows={8} /></Form.Item>
        <Form.Item name="topics" label="Topics"><Input /></Form.Item>
