...
import type { AddPathPayload, AddTextPayload, Document, KnowledgeResponse, ReplaceSourceFilePayload, ReplaceSourcePayload, SearchResult, UpdateDocumentPayload, UploadDocumentPayload } from './types'
...
export const uploadDocument = ({ file, title, source, visibility }: UploadDocumentPayload) => {
...
export const replaceDocumentSourceFile = (id: string, { file }: ReplaceSourceFilePayload) => {
  const body = new FormData()
  body.append('file', file, file.name)
