...
import { KNOWLEDGE_FILE_ACCEPT, knowledgeFileErrorMessage, normalizeUploadFiles, selectedUploadFile, validateKnowledgeFile } from '../utils'
...
export function DocumentDrawer({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (document: Document) => Promise<void> }) {
  ...
  return <Drawer size={560} open={open} onClose={onClose} destroyOnHidden title="添加知识文档">
    <Tabs destroyOnHidden items={[
      { key: 'upload', label: 'Upload', children: <Form layout="vertical" preserve={false} initialValues={{ visibility: 'private' }} onFinish={(values: { fileList: UploadFile[]; title?: string; source?: string; visibility: ResourceVisibility }) => {
        const file = selectedUploadFile(values.fileList)
        if (!file) return
        return submit(() => uploadDocument({ file, title: values.title, source: values.source, visibility: values.visibility }))
      }}>
      ...
      { key: 'text', label: 'Text', children: <Form layout="vertical" preserve={false} initialValues={{ visibility: 'private' }} onFinish={(values: { title: string; source?: string; content: string; visibility: ResourceVisibility }) => submit(() => addText(values))}>
