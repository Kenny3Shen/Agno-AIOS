...
export function DocumentDrawer({ open, onClose, onCreated, ingestDefaults }: { open: boolean; onClose: () => void; onCreated: (document: Document) => Promise<void>; ingestDefaults?: KnowledgeIngestDefaults }) {
  ...
  return <Drawer size={560} open={open} onClose={onClose} destroyOnHidden title="添加知识文档">
    <Tabs destroyOnHidden items={[
      { key: 'upload', label: 'Upload', children: <Form layout="vertical" preserve={false} initialValues={{ visibility: 'private' }} onFinish={(values: { fileList: UploadFile[]; title?: string; source?: string; visibility: ResourceVisibility; ingest_options?: KnowledgeIngestOptions }) => {
        const file = selectedUploadFile(values.fileList)
        if (!file) return
        return submit(() => uploadDocument({ file, title: values.title, source: values.source, visibility: values.visibility, ingest_options: cleanIngestOptions(values.ingest_options) }))
      }}>
