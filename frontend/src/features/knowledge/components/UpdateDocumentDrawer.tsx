...
import { buildMetadataUpdate, hasMetadataUpdate, KNOWLEDGE_FILE_ACCEPT, knowledgeFileErrorMessage, normalizeUploadFiles, replacementFileName, selectedUploadFile, validateKnowledgeFile } from '../utils'
...
export function UpdateDocumentDrawer({ document, open, onClose, onUpdated }: { document: Document; open: boolean; onClose: () => void; onUpdated: (document: Document) => Promise<void> }) {
  ...
  return <Drawer size={560} open={open} onClose={onClose} destroyOnHidden title="更新文档">
    <Tabs destroyOnHidden items={[
      ...
      { key: 'upload', label: 'Upload', children: <Form layout="vertical" preserve={false} onFinish={(values: { fileList: UploadFile[] }) => {
        const file = selectedUploadFile(values.fileList)
        if (!file) return
        return submit(() => replaceDocumentSourceFile(document.id, { file }), '文件已更新并重新向量化')
      }}>
        <Alert className="knowledge-form-note" type="warning" showIcon title="上传文件会替换当前文档源内容，保留当前 Metadata，并重新分块、重新向量化。" />
      ...
      { key: 'text', label: 'Text', children: <Form layout="vertical" initialValues={{ file_name: replacementFileName(document), content: '' }} onFinish={(values: { file_name: string; content: string }) => submit(() => replaceDocumentSource(document.id, { file_name: values.file_name.trim(), content: values.content }), '内容已更新并重新向量化')}>
