...
export const createStep = (kind: Step['kind'] = 'agent'): Step => ({
...
export const moveStep = (steps: Step[], id: string, direction: -1 | 1) => {
...
export const buildWorkflowCode = (state: WorkflowState) => {
  const steps = state.steps
    .map(
      (step) =>
        `    ${JSON.stringify({ name: step.name || step.targetId, executor_type: step.kind, executor_id: step.targetId, instructions: step.instructions })},`
    )
    .join('\n')
