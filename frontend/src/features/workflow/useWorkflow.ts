...
export function useWorkflow() {
  ...
  const add = (kind: Step['kind']) => {
    ...
    setState((current) => ({ ...current, steps: [...current.steps, step], selectedId: step.id }))
  }
  const update = (step: Step) =>
    setState((current) => ({ ...current, steps: current.steps.map((item) => (item.id === step.id ? step : item)) }))
