...
import { Alert, Button, Card, Col, DatePicker, Empty, Flex, Row, Segmented, Space, Statistic, Table, Tag, Tooltip, Typography, theme } from 'antd'
...
export function DashboardPage() {
  ...
  const overviewQuery = useMemo<OverviewQuery>(() => customRange
  ...
  return <main className={`page dashboard-page dashboard-data-${dataMotion}`}>
    <PageHeader
      title="运行概览"
      description="以运行时间轴聚合 Trace、质量与治理信号"
      actions={<Space wrap>
        <Segmented value={customRange ? undefined : range} options={[...runtimeRanges]} onChange={(value) => { setCustomRange(null); setRange(value as OverviewRange) }} />
        <RangePicker aria-label="自定义时间段" showTime value={customRange} onChange={(value) => setCustomRange(value as [Dayjs, Dayjs] | null)} />
        <Tooltip title="刷新"><Button aria-label="刷新运行概览" icon={<ReloadOutlined />} loading={query.isFetching} onClick={() => void query.refetch()} /></Tooltip>
      </Space>}
    />
    ...
    <section className="dashboard-signal-rail" aria-label="运行状态">
      <span><i className={healthClassName} />Runtime <b>{healthStatus}</b></span>
      <span>观察窗口 <b>{customRange ? `${formatDate(customRange[0].toISOString())} — ${formatDate(customRange[1].toISOString())}` : range}</b></span>
