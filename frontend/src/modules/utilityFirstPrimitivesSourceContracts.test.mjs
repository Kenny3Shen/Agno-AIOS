...
assert.match(statusChip, /<el-tag[\s\S]*:type="tagType"/, "StatusChip must delegate status semantics to Element Plus tags")
assert.match(emptyState, /ag-empty-state/, "EmptyState must consume the shared empty state shortcut")
assert.match(emptyState, /<el-empty/, "EmptyState must delegate empty-state structure to Element Plus")
...
assert.match(knowledgeDocumentList, /<el-table[\s\S]*<el-pagination/, "Knowledge document list must use Element Plus table and pagination primitives")
...
assert.doesNotMatch(knowledgeMetadataPanel, /DataChip/, "Knowledge metadata panel must reserve chips for compact status data")
...
assert.match(knowledgeMetadataPanel, /<el-descriptions[\s\S]*v-for="item in metadataItems"/, "Knowledge metadata panel must render label/value facts through Element Plus descriptions")
