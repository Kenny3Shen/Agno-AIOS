...
模型供应商支持 DeepSeek / OpenAI / **xAI（Agno 官方 `xAI` 类，Chat Completions）** / OpenAI-compatible。 xAI 可配置 structured output 与 Live Search；Chat 输入区可开关联网搜索与知识库检索。历史 Grok 配置（`api.x.ai` 或 `model_id` 以 `grok` 开头）加载时自动迁移为 `provider=xai`，不再走 OpenAI Responses。
