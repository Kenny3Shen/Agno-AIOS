...
React 工作台通过共享 API client 携带 token 请求 FastAPI；后端检查权限和资源归属后，按模型、MCP、Skills、Knowledge 与 Memory 配置创建 Agno 运行时。Chat 通过 SSE 返回流式输出（含模型层 `run.retrying` 重试提示）；Markdown 启用 KaTeX（`$` / `$$` / `\\[ \\]`）渲染公式；`GET /api/chat/sessions` 使用 Agno 风格 `data`/`meta`（默认 `limit=100`、硬顶 500；DB 真分页 + 归档 SQL 过滤，行字段仍为工作台 session 摘要）。Overview 评估快照用近期样本（`sample_size`）算 pass_rate，文档数走 paged total；traces 最多采样 5000；submissions 为 data/meta。Trace、Memory 和 Knowledge 等视图通过 Query 刷新读取最新数据。
