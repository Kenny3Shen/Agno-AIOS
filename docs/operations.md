...
- CVE 情报源配置为仓库根目录 `cve_sources.toml`（可用 `TAIS_CVE_SOURCE_CONFIG_PATH` 覆盖）。IP 黑名单威胁情报源为 `ip_blacklist_sources.toml`（默认 FireHOL level1；`TAIS_IP_BLACKLIST_SOURCE_CONFIG_PATH` 可覆盖）；管理员可在「安全情报 → IP 黑名单」更新库，Chat 意图可挂载 `ip-blacklist-skill`。
...
- Chat 运行参数（Alembic `20260721_0007` + `0008` + `0009` 扩展 `chat_settings`）：**Settings → Chat 设置** 可配 Agno 常用 knobs——`num_history_runs`、`session_summaries_enabled`、`add_datetime_to_context`、`max_tool_calls_from_history`、`default_tool_call_limit`、`enable_agentic_memory`（默认关）、`markdown`；Memory P0：`memory_tool_content_enabled`、`memory_prune_*`；Memory P1 注入筛选：`memory_inject_enabled` / `memory_inject_top_k` / `memory_inject_max_chars` / `memory_inject_window_days` / `memory_inject_dedupe_topics`（在 Agno 写入 system prompt 前按 0.4 新近度 + 0.35 关键词 + 0.15 topics + 0.1 质量打分并截断）。Chat/Team 使用独立廉价 `MemoryManager`；无真实 `user_id` 时 fail-closed。API 仍为 `GET/PATCH /api/settings/chat`。
