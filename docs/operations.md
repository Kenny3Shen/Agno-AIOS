...
- Chat 运行参数（Alembic `20260721_0007` + `20260721_0008` 扩展 `chat_settings`）：**Settings → Chat 设置** 可配 Agno 常用 knobs——`num_history_runs`、`session_summaries_enabled`、`add_datetime_to_context`、`max_tool_calls_from_history`、`default_tool_call_limit`、`enable_agentic_memory`（默认关）、`markdown`，以及 Memory P0：`memory_tool_content_enabled`（默认关，是否从工具结果抽取长期记忆）、`memory_prune_enabled` / `memory_prune_retention_days` / `memory_prune_top_k`（durable job 按 `updated_at` 删过期 + 算法 Top-k）。Chat/Team 使用独立廉价 `MemoryManager`；无真实 `user_id` 时 fail-closed 不写记忆。API 仍为 `GET/PATCH /api/settings/chat`。
