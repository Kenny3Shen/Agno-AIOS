...
- Chat 运行参数（Alembic `20260721_0007` 扩展 `chat_settings`）：**Settings → Chat 设置** 可配 Agno 常用 knobs——`num_history_runs`、`session_summaries_enabled`、`add_datetime_to_context`、`max_tool_calls_from_history`、`default_tool_call_limit`、`enable_agentic_memory`、`markdown`（外加隐私/时间线开关）。作用于 Chat Agent 与 Team leader；API 仍为 `GET/PATCH /api/settings/chat`。
