...
- Dashboard Overview 热路径：同一进程内、同一权限投影和查询窗口的重叠请求会合并为一次构建，并缓存 3 秒（最多 256 个键、32 个在途构建）；缓存值在返回前深拷贝，不能被调用方污染。它不跨进程，也不替代写入后的实时一致性；压测应在缓存命中和错峰 miss 两种情形下分别观察。
- Chat 历史与流式：`GET /api/chat/sessions/{session_id}?limit=40` 返回最近的完整 top-level turns，`meta.next_cursor` 传给 `before` 读取更早窗口（`limit` 为 1–100，旧的不带分页参数的请求仍返回完整数组）。生产前端初始只取 40 turns，向上加载时保持可见锚点；最新页刷新会重排已加载的有界窗口、不会丢失页边界消息。SSE 更新按动画帧合并，终态立即 flush；性能回归应同时覆盖长历史分页与高频 token 流。
...
- Trace 索引治理：`uv run audit-trace-indexes` 默认只读核验 `AGNO_DB_SCHEMA.agno_traces` 的表统计、现有索引及两项批准组合索引，并输出建议 SQL；`uv run audit-trace-indexes --explain` 会在 `READ ONLY` 事务内以 15 秒上限采集脱敏的 `EXPLAIN (ANALYZE, BUFFERS)` 证据。`agno` 的 trace/session 表不属于普通 Alembic 控制面；只有先审阅报告后显式执行 `uv run audit-trace-indexes --apply --confirm CREATE_AGNO_TRACE_INDEXES`，才会以 autocommit 的 `CREATE INDEX CONCURRENTLY IF NOT EXISTS` 创建缺失的 `(user_id, start_time DESC)`、`(user_id, status, start_time DESC)` 索引。命名冲突或无效同名索引会 fail closed，需人工处理；生产执行前应评估写放大、锁与磁盘余量。
...
正式产品不捆绑供应商密钥、测试模型或 `builtin` 模型行。全新数据库的模型表为空是合法状态，`GET /api/models` 会返回空列表且不写入 seed；管理员必须在 **Settings → 模型连接** 自行添加连接、补全密钥与 Model ID、启用并保存为当前模型。首个保存的模型会成为当前模型。没有已启用且配置完整的模型时，Chat、Workflow 与评测的模型调用会明确拒绝执行，而不是隐式回退到 SDK 默认模型。
...
模型连接可指定 **MemoryManager** 模型（`memory_model_id` / 行级 `memory_manager` 标记，Alembic `20260721_0012`）：用于记忆抽取的廉价模型；未指定时自动挑选 flash/mini 等。Chat 的 subject 使用 `active_model_id`。**Eval Judge**（`eval_judge_model_id`）是可选的专用评测连接；未钉选时 `AccuracyEval` 和 `AgentAsJudgeEval` 使用已配置的 `active_model_id`。两类 LLM 判定必须解析到已启用、完整配置的连接，绝不使用 Agno 进程默认模型；仅 Reliability/Performance 的判定本身不需要 Judge，但被测 Agent 仍需要当前模型。
