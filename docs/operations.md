...
- Chat 运行参数（Alembic `20260721_0007`–`0010` 扩展 `chat_settings`）：**Settings → Chat 设置** 可配 Agno knobs（history / summaries / tools / markdown）与 Memory：
  - **记忆模式** `memory_mode`：`off` | `automatic` | `agentic`（Settings 单选，禁止双开关误开）；遗留字段 `memory_enabled` / `enable_agentic_memory` 由模式推导。
  - Capture：全局只记偏好/职责/长期事实；`security-operations` 附加 SOC 白名单（禁记 IOE/临时 IP/告警细节；运维事实走 Knowledge）。
  - P0：`memory_tool_content_enabled`、`memory_prune_*`；P1 注入：`memory_inject_*`。
  Chat/Team 使用廉价 `MemoryManager`；无真实 `user_id` 时 fail-closed。API：`GET/PATCH /api/settings/chat`。
