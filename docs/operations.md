...
- 模型输入护栏（Agno `pre_hooks`，不含 OpenAI Moderation）：`TAIS_GUARDRAILS_ENABLED`（默认 true）、`TAIS_GUARDRAILS_PII_ENABLED`、`TAIS_GUARDRAILS_PII_MASK`（默认 false=拦截）、`TAIS_GUARDRAILS_PII_CHECK_EMAIL`（默认 false）、`TAIS_GUARDRAILS_PII_CHECK_PHONE`、`TAIS_GUARDRAILS_PROMPT_INJECTION_ENABLED`。Chat/Team leader/Workflow 步骤 Agent 共享同一套；SSE 失败码形如 `GUARDRAIL_PII_DETECTED` / `GUARDRAIL_PROMPT_INJECTION`（`retryable=false`）。
