...
- `api.services.os_control_service`：控制面 payload 入口已改为 awaitable API；sessions/memory/scheduler 使用 Agno async APIs，trace、span、knowledge 状态和 AIOS control table projection 使用 async Postgres pool，避免 FastAPI route 中执行同步 DB I/O。这些直接读取是 AgentOS control payload 的 async product view/API gap，不改变 Agno table ownership。
