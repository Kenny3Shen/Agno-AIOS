...
- Page-level Memory/Approvals APIs：`api.routes.memory` 和 `api.routes.approvals` 分别承载 Memory 与 Approvals 页面权限边界，不再保留 `os_*_control` facade 或通用 `/api/os/*` control 入口。Memory 读取、更新和删除通过 Agno memory APIs 完成，并保留 owner scoping、growth signals 和 audit；Approvals route 保留 pending count、filter normalization、resolver identity 和 audit。Scheduler 继续由前端直接调用 AgentOS `/schedules` API。
...
- 旧 `os_metrics_control` / `os_knowledge_control` lightweight dashboard projection 已删除；Trace、Dashboard 和 Knowledge 页面直接通过各自页面 API 读取需要的数据。
