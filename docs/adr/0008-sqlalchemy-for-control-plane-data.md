...
Agno PgVector 文档推荐在 async Knowledge flow 中构造 `PgVector(table_name=..., db_url="postgresql+psycopg://...")` 并调用 `Knowledge.ainsert()` / `Knowledge.asearch()`。本地安装的 Agno 版本中，`PgVector.async_create()`、`PgVector.async_search()` 和 async write helpers 仍会经过同步 SQLAlchemy engine 或同步 session。AIOS 因此提供 `AsyncPgVector` adapter：继承 Agno `PgVector` 以保留 table contract、reader/Knowledge integration 和 score semantics，但覆盖 `async_create()`、`async_insert()`、`async_upsert()` 和 `async_search()`，按 PgVector `db_url` 派生 `postgresql+psycopg_async://` async engine 执行 table I/O，并在 FastAPI lifespan shutdown 时释放 PgVector async engines。这样 AIOS 仍符合 Agno 的 Knowledge async API 形态，同时避免 request path 落回 thread-backed vector DB I/O。本地 SentenceTransformer embedding 和 FlagEmbedding rerank 不是 DB I/O；这些同步模型计算通过 `anyio.to_thread.run_sync` 隔离到 worker thread，避免阻塞 FastAPI/Knowledge async event loop。
