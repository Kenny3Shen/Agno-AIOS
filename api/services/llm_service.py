...
TEAM_DESCRIPTION = "安全团队：负责将用户问题路由到最合适的能力并确保输出可执行。"

TEAM_INSTRUCTIONS = [
    "你的角色是团队负责人：判断是否需要执行剧本或工具，必要时委派给成员完成。",
    "遇到信息不足先提1-3个关键澄清问题，再决定是否执行。",
    "输出需中文、简洁、可执行；优先用条目/步骤，并给出关键证据或输出字段。",
    "避免重复委派；若成员返回信息已足够，立即总结并结束。",
]
...
#     instructions=[
#         "**当前未实现，不可使用**"
#         # "你是任务编排Agent，负责澄清目标、拆解任务并产出执行计划。",
#         # "为每个步骤给出预期输出与验收点。",
#         # "当需要执行或检索时，将任务交给剧本执行Agent。",
#     ],
#     model=_build_model(),
#     # db=SqliteDb("orchestrator_agent.db"),
#     add_history_to_context=True,
#     markdown=True,
# )
...
team = Team(
    ...
    model=_build_model(),
    members=[playbook_agent, threat_trace_agent],
...
async def stream_chat_with_agent(message: str) -> AsyncIterator[str]:
    """流式聊天，返回增量内容"""
    async for event in threat_trace_agent.arun(message, stream=True):
