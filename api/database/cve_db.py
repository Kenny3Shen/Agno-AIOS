...
async def search_cves_by_id_paginated(
    ...
    async with pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            offset = (page - 1) * size
            sql = "SELECT * FROM cves WHERE cve_id LIKE %s ORDER BY id DESC LIMIT %s OFFSET %s"
...
async def search_cves_by_description_paginated(
    ...
    async with pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            offset = (page - 1) * size
            sql = "SELECT * FROM cves WHERE description LIKE %s ORDER BY id DESC LIMIT %s OFFSET %s"
