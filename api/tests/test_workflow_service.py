...
def test_row_payload_canonicalizes_legacy_definition() -> None:
    payload = _row_payload(
        {
            "definition": {
                "steps": [
    ...
    assert payload["definition"]["steps"][0]["executor"] == {
        "kind": "agent",
        "ref": "security-operations",
    }
...
def test_get_published_definition_canonicalizes_retired_skill() -> None:
...
@pytest.mark.asyncio
async def test_list_versions_canonicalizes_historic_retired_skill() -> None:
    ...
    with (
        ...
        result = await workflow_service.list_versions_for_actor(actor, "wf-1")

    assert result is not None
    assert result["data"][0]["definition"]["steps"][0]["skills"] == [
        "cve-intel-skill"
    ]


@pytest.mark.asyncio
async def test_restore_version_canonicalizes_historic_retired_skill_before_update() -> None:
    actor = SimpleNamespace(id="user-1", role="user", is_superuser=False)
    version = {
        "name": "Historic workflow",
        "description": "",
    ...
    with (
        ...
        result = await workflow_service.restore_version_for_actor(actor, "wf-1", 2)

    assert result == {"id": "wf-1"}
    update_call = update.await_args
    assert update_call is not None
    restored_definition = update_call.kwargs["definition"]
    assert restored_definition["steps"][0]["skills"] == ["cve-intel-skill"]
